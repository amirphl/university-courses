...
func DSAPrivateKeySample() *dsa.PrivateKey {
	return &dsa.PrivateKey{
		PublicKey: *DSAPublicKeySample(),
...
func DSAPublicKeySample() *dsa.PublicKey {
	return &dsa.PublicKey{
		Parameters: dsa.Parameters{
			P: new(big.Int).SetInt64(23),
			Q: new(big.Int).SetInt64(11),
			G: new(big.Int).SetInt64(16),
		},
...
func sign(rand io.Reader, priv *dsa.PrivateKey, hash uint64) (*dsaSignature, error) {
	...
	if priv.Q.Sign() <= 0 || priv.P.Sign() <= 0 || priv.G.Sign() <= 0 || priv.X.Sign() <= 0 {
...
func main() {
	...
	privKey := DSAPrivateKeySample()
	pubKey := DSAPublicKeySample()
