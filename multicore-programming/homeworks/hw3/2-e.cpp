...
void omp_check();
void fill_array(int *a, int n, int range);
void print_array(int *a, int n);
void function(int *a, int n, int *c, int range);
...
int main(int argc, char *argv[]) {
	...
	int * c = (int *)malloc((85 + 1) * sizeof(int));
	
	for (int j = 0;j<85 + 1;j++)
	{
		c[j] = 0;
	}
	
	fill_array(a, n, range);
...
void insertionSort(int a[], int n)
{
		...
		for(int i = id * n / 4; i < (id + 1) * n / 4; i++)
		{
            int k = a[i];
			int j;
            for (j = i; j > id * n / 4 && a[j-1] > k; j--)
                    a[j] = a[j-1];
            a[j] = k;
		}
		...
		#pragma omp barrier
		if(id == 0){
			merge(a,0,(n / 4) - 1, (2 * n / 4) - 1);
		}
		if(id == 1){
			merge(a,(2 * n / 4),(3 * n / 4) - 1, (4 * n / 4) - 1);
		}
		
		#pragma omp barrier
		if(id == 2){
			merge(a,0,(2 * n / 4) - 1,(4 * n / 4) - 1);
		}	
}
